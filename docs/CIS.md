...
**Status:** Draft  
**Aligned with:** Cellrix White Paper v2.4  
**Date:** 2026-05-16
...
| **On‑Demand Declaration** | Intent producers shall output only the Manifest content required by the current task. Do not pre‑generate interface components that may be needed in the future; do not assume the capabilities of the Runtime. All interaction channels are closed by default and must be explicitly declared through the `actions` field in the Manifest. |
